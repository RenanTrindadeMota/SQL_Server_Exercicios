...
--EXERCÍCIO 6:
SELECT Clientes.NomeCliente, COUNT(DISTINCT DetalhesPedidos.ProdutoID) AS ComprasDistintas
FROM Clientes
INNER JOIN Pedidos ON Clientes.ClienteID = Pedidos.ClienteID
INNER JOIN DetalhesPedidos ON Pedidos.PedidoID = DetalhesPedidos.PedidoID
INNER JOIN Produtos ON DetalhesPedidos.ProdutoID = Produtos.ProdutoID
...
--EXERCÍCIO 7:
SELECT Pedidos.PedidoID, AVG(DetalhesPedidos.Quantidade) AS MédiaItensPorPedido
FROM Pedidos
INNER JOIN DetalhesPedidos ON Pedidos.PedidoID = DetalhesPedidos.PedidoID
...
SELECT
	...
	COUNT(Pedidos.PedidoID) AS QuantidadePedidos
FROM 
	Pedidos
...
SELECT
	NomeCategoria,
	SUM(DetalhesPedidos.Quantidade * Produtos.Preco) AS TotalPedido
FROM 
	Categorias
INNER JOIN Produtos
ON 
...
SELECT
	NomeCliente,
	MAX(Pedidos.Total) 'Total por Pedido'
FROM
	Clientes
INNER JOIN Pedidos
ON 
